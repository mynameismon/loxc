...
let add_token ?(current = 1) context token =
  let new_current = context.current + current in
  {
    ...
    start = new_current;
    tokens =
      { kind = token; line_no = context.line; col = context.start }
      :: context.tokens;
  }
...
let scan_num chars context =
  let rec scan_int curr_str chars context =
    match chars with
    | ('0' .. '9' as num) :: t ->
        scan_int (num :: curr_str) t
          { context with current = context.current + 1 }
    | _ -> (curr_str, chars, context)
  in
  let pre, rem, context = scan_int [] chars context in
  let post, rem, context =
    match rem with
    | '.' :: ('0' .. '9' as digit) :: t ->
        scan_int ('.' :: pre) (digit :: t)
          { context with current = context.current + 1 }
    | _ -> (pre, rem, context)
  in
  let token =
    ...
    | _ -> Tokens.Number (Float.of_string (char_to_string post))
  in
  (rem, add_token ~current:0 context token)

let rec lex_comments chars =
  match chars with [] -> chars | '\n' :: _ -> chars | _ :: t -> lex_comments t
...
let rec scan_token remaining context =
  ...
  | '\n' :: t ->
      scan_token t
        { context with line = context.line + 1; start = 1; current = 1 }
  | ' ' :: t | '\t' :: t | '\r' :: t ->
      scan_token t
        {
          context with
  ...
  | ';' :: t -> scan_token t (add_token context Tokens.Semicolon)
  | '!' :: '=' :: t ->
      scan_token t (add_token ~current:2 context Tokens.BangEqual)
  | '!' :: t -> scan_token t (add_token context Tokens.Bang)
  | '=' :: '=' :: t ->
      scan_token t (add_token ~current:2 context Tokens.EqualEqual)
  | '=' :: t -> scan_token t (add_token context Tokens.Equal)
  | '>' :: '=' :: t ->
      scan_token t (add_token ~current:2 context Tokens.GreaterEqual)
  | '>' :: t -> scan_token t (add_token context Tokens.Greater)
  | '<' :: '=' :: t ->
      scan_token t (add_token ~current:2 context Tokens.LessEqual)
  | '<' :: t -> scan_token t (add_token context Tokens.Less)
  | '/' :: '/' :: t ->
      scan_token (lex_comments t)
        { context with line = context.line + 1; start = 1; current = 1 }
      (* Handle comments gracefully *)
  | '/' :: t -> scan_token t (add_token context Tokens.Slash)
  | '"' :: t ->
      let tokens, context = scan_string [] t context in
      scan_token tokens context
  | '0' .. '9' :: _ ->
      let tokens, context = scan_num remaining context in
      scan_token tokens context
  | 'a' .. 'z' :: _ | 'A' .. 'Z' :: _ | '_' :: _ ->
      let tokens, context = scan_identifier remaining context in
      scan_token tokens context
  | ':' :: t ->
      scan_token t (add_token context Tokens.Colon)
      (* Lox Extension: Lexing for type checking *)
  | h :: t ->
      scan_token t
        (add_token context
           (Error (LexError (Printf.sprintf "Unknown token %c" h))))

let string_to_list s = s |> String.to_seq |> List.of_seq
let init_context = { line = 1; current = 1; start = 1; tokens = [] }

let scan_tokens program =
  List.rev (scan_token (string_to_list program) init_context).tokens
...
let%test "lex nothing" = scan_tokens "" = []
...
let%test "lex multiple numbers" =
  scan_tokens "1 2 3"
  = [
      { kind = Tokens.Number 1.; line_no = 1; col = 1 };
...
let%test "lex string" =
  ...
  = [ { kind = Tokens.String "Hello world"; line_no = 1; col = 1 } ]

let%test "incomplete string" =
  scan_tokens "\"Hello"
  = [
      {
        kind = Tokens.Error (Error.LexError "String not closed!");
        line_no = 1;
        col = 6;
      };
    ]
...
let%test "lex groupings" =
  scan_tokens "(1 + 2)"
  = [
      { kind = Tokens.LeftParen; line_no = 1; col = 1 };
